...
let tests =
  ...
  >::: [ ("test1"
       ...
       ; ("test4"
          ...
          let actual = string_of_program program in
          let expected =
            "bind info<Person>(self: Person, ) -> string {\n\
             return self.name + self.age;\n\n\
             }\n"
          in
          assert_equal expected actual ~printer:(fun s -> "\"" ^ s ^ "\""))
       ; ("test5"
          >:: fun _ ->
          let lexbuf =
            Lexing.from_string "let p2 := Person::new(\"John\", 12);\np.info();\n"
          in
          let program = Parser.program_rule Scanner.tokenize lexbuf in
          let actual = string_of_program program in
       ...
       ; ("test6"
          >:: fun _ ->
          let lexbuf =
            Lexing.from_string "bind increase_age<Person>(self) {\n\tself.age += 1;\n}\n"
          in
          let program = Parser.program_rule Scanner.tokenize lexbuf in
          let actual = string_of_program program in
          let expected =
            "bind increase_age<Person>(self: Person, ) -> () {\nself.age += 1;\n\n}\n"
          in
          assert_equal expected actual ~printer:(fun s -> "\"" ^ s ^ "\""))
       ; ("test7"
          >:: fun _ ->
          ...
          let actual = string_of_program program in
          let expected =
            "bind change_age<Person>(self: Person, new_age: int, ) -> () {\n\
             self.age = new_age;\n\n\
             }\n"
          in
          assert_equal expected actual ~printer:(fun s -> "\"" ^ s ^ "\""))
