...
let tests =
  ...
  >::: [
    ("udt_struct_definition"
     >:: fun _ ->
      let sast = get_sast "type Person { name : string, age : int } fun main() -> () { let dummy := Person { name : \"A\", age : 1 }; }" in
    ...
    ("udt_instance_and_field_store"
     >:: fun _ ->
      let sast = get_sast "type Person { name : string, age : int } fun main() -> () { let p := Person { name : \"Alice\", age : 30 }; }" in
      let mdl = Irgen.translate sast in
      let actual = L.string_of_llmodule mdl in
      assert_bool "alloca for Person instance" (str_contains actual "alloca { i8*, i32 }");
      assert_bool "store name field" (str_contains actual "store i8* getelementptr inbounds ([6 x i8], [6 x i8]* @str_" );
      assert_bool "store age field" (str_contains actual "store i32 30");
    );
    ("udt_field_access"
     >:: fun _ ->
      let sast = get_sast "type Person { name : string, age : int } fun main() -> int { let p := Person { name : \"Bob\", age : 42 }; return p.age; }" in
      let mdl = Irgen.translate sast in
      let actual = L.string_of_llmodule mdl in
    ...
    ("udt_as_function_argument"
     >:: fun _ ->
      let sast = get_sast "type Point { x : int, y : int } fun print_point(p : Point) -> () {}" in
      let mdl = Irgen.translate sast in
      let actual = L.string_of_llmodule mdl in
      assert_bool "function takes Point argument" (str_contains actual "define void @print_point({ i32, i32 } %0)");
    );
  ]
;;
