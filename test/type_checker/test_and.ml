...
let tests =
  ...
  >::: [
    ("bool_and_bool" >:: fun _ ->
      let actual = check_program "fun main() {let a := true;\nlet b := false;\n a && b;}" in
      let expected = "" in
    ...
    ("bool_and_char" >:: fun _ ->
      let actual = check_program "type Person {name: string, age: int} fun main() {let a := true;\nlet b := Person {name: \"John\", age: 25};\n a && b;}" in
