...
let tests =
  ...
  >::: [
    ("int_div_int" >:: fun _ ->
      let actual = check_program "fun main() -> int {let a := 5;\nlet b := 1;\nreturn a/b;}" in
      let expected = "" in
      assert_equal expected actual ~printer:(fun s -> "\"" ^ s ^ "\""));
    
    ("float_div_float" >:: fun _ ->
      let actual = check_program "fun main() -> float {let a := 5.0;\nlet b := 1.0;\na/b;}" in
      let expected = "" in
      assert_equal expected actual ~printer:(fun s -> "\"" ^ s ^ "\""));
    ...
    ("bool_div_int" >:: fun _ ->
      let actual = check_program "fun main() -> int {let a := true;\nlet b := 5;\na/b;}" in
    ...
    ("string_div_int" >:: fun _ ->
      let actual = check_program "fun main() {let a := \"nonsense\";\nlet b := 5;\n a/b;}" in
      let expected = "Expression: 'a / b' LHS: string, RHS: int" in
      assert_equal expected actual ~printer:(fun s -> "\"" ^ s ^ "\""));

    ("udt_div_int" >:: fun _ ->
      let actual = check_program "type Person {name: string, age: int} fun main() {let a := Person {name: \"John\", age: 25};\nlet b := 5;\n a/b;}" in
