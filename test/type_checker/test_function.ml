...
let tests =
  ...
  >::: [ ("normal_function"
       ...
       ; ("recursive_function"
          ...
          let actual =
            check_program
              "fun function(n: int) -> int { if (n == 0) { return 1; } return function(n \
               - 1); } fun main() -> int { return function(5); }"
          in
          let expected = "" in
          assert_equal expected actual ~printer:(fun s -> "\"" ^ s ^ "\""))
       ; ("call_nested_function"
          ...
          let actual =
            check_program
              "fun function() -> int { fun nested() -> () { return; } return 0; } fun \
               main() -> int { return nested(); }"
          in
       ...
       ; ("more_recursion"
          ...
          let actual =
            check_program
              "fun outer(m: int) -> int { fun nested(n: int) -> int { if (n == 0) \
               {return 1;} return 1 + nested(n - 1); } return nested(m); } fun main() -> \
               int { return outer(5); }"
          in
          let expected = "" in
          assert_equal expected actual ~printer:(fun s -> "\"" ^ s ^ "\""))
