...
let tests = "testing_func_bind" >::: [
        ...
        let expected = "TYPE ID(Person) LBRACE ID(name) COLON STRING COMMA ID(age) COLON INT RBRACE" in
        assert_equal 
        expected actual
        ~printer:(fun s -> "\"" ^ s ^ "\""));
        ...
        let actual = List.map print_token (to_list lexbuf) |> String.concat " " in
        let expected = "BIND ID(new) LT ID(Person) GT LPAREN ID(name) COLON STRING COMMA ID(age) COLON INT RPAREN ARROW ID(Person) LBRACE RETURN ID(Person) LBRACE ID(name) COLON ID(name) COMMA ID(age) COLON ID(age) RBRACE SEMI RBRACE" in
        assert_equal 
        expected actual
        ~printer:(fun s -> "\"" ^ s ^ "\""));
        ...
        let actual = List.map print_token (to_list lexbuf) |> String.concat " " in
        let expected = "BIND ID(info) LT ID(Person) GT LPAREN SELF RPAREN ARROW STRING LBRACE RETURN SELF DOT ID(name) PLUS SLIT( ) PLUS SELF DOT ID(age) SEMI RBRACE" in
        assert_equal 
        expected actual
        ~printer:(fun s -> "\"" ^ s ^ "\""));
