...
let tests = "testing_func_def_and_call2" >::: [
        ...
        let actual = List.map print_token (to_list lexbuf) |> String.concat " " in
        let expected = "FUN ID(map) LPAREN ID(f) COLON FUN ID(a) ARROW ID(b) RPAREN ARROW FUN LIST LT ID(a) GT ARROW LIST LT ID(b) GT LBRACE RETURN MATCH ID(l) LBRACE LBRACKET RBRACKET ARROW LBRACKET RBRACKET COMMA ID(head) DCOLON ID(tail) ARROW ID(map) LPAREN ID(f) RPAREN LPAREN ID(tail) RPAREN COMMA RBRACE RBRACE" in
        assert_equal 
        expected actual
        ~printer:(fun s -> "\"" ^ s ^ "\""));
        ...
        let actual = List.map print_token (to_list lexbuf) |> String.concat " " in
        let expected = "FUN ID(typecast) LPAREN ID(x) COLON INT RPAREN ARROW FLOAT LBRACE RETURN FLOAT LPAREN ID(x) RPAREN SEMI RBRACE" in
        assert_equal 
        expected actual
        ~printer:(fun s -> "\"" ^ s ^ "\""));
        ...
        let actual = List.map print_token (to_list lexbuf) |> String.concat " " in
        let expected = "LET ID(float_lst) WALRUS ID(map) LPAREN ID(lst) RPAREN LPAREN ID(typecast) RPAREN SEMI" in
        assert_equal 
        expected actual
        ~printer:(fun s -> "\"" ^ s ^ "\""));
