...
let ltype_of_typ = function
  ...
  | A.Unit -> l_unit
  | A.UserType name ->
      (try Hashtbl.find udt_structs name
       with Not_found -> raise (Failure ("Unknown user type: " ^ name)))
  | A.String -> l_str
  | t ->
...
let build_udt_access typ var_name field_name vars builder =
  ...
  let var_typ = match typ with
    ...
    | _ -> raise (Failure "expected user-defined type") in
  let field_indices = Hashtbl.find udt_field_indices var_typ in
  let idx = List.assoc field_name field_indices in
  let field_ptr = L.build_struct_gep struct_ptr idx (var_name ^ "_" ^ field_name) builder in
...
let rec build_expr expr vars var_types the_module builder =
  ...
  | SUDTInstance (typename, fields) ->
    ...
    List.iter (fun (field_name, (_, sx)) ->
      let idx = List.assoc field_name field_indices in
      let field_ptr = L.build_struct_gep instance idx (typename ^ "_" ^ field_name) builder in
  ...
  | SUDTAccess (id, SUDTVariable field) ->
    ...
    let type_name = match id_typ with
      | A.UserType n -> n
    ...
    field_val
  | SStringLit s ->
    let str_ptr = L.build_global_stringptr s ("str_" ^ (Digest.to_hex (Digest.string s))) builder in
    str_ptr
  | e ->
...
let add_local_val typ var vars var_types (expr : A.typ * Sast.sx) the_module builder =
  ...
  let sx = snd expr in
  let ll_initializer_value : L.llvalue = build_expr sx vars var_types the_module builder in
  match typ with
...
let translate blocks =
  ...
  and process_func_block (func_block : L.llvalue * A.formal list * sblock list) vars var_types =
  ...
  and process_blocks blocks vars var_types (curr_func : L.llvalue option) func_blocks (builder : L.llbuilder option) =
    match blocks with
  ...
  and process_block block vars var_types (curr_func : L.llvalue option) func_blocks (builder : L.llbuilder option) =
    match block with
    ...
    | SDeclTyped (id, typ, expr) ->
      ...
      then
        ( let new_vars = add_local_val typ id vars var_types expr the_module (Option.get builder) in
    ...
    | SExpr expr ->
      ...
      (vars, var_types, curr_func, func_blocks, builder)
    | SIfEnd (expr, blks) ->
      let bool_val = build_expr (snd expr) vars var_types the_module (Option.get builder) in
    ...
    | SIfNonEnd (expr, blks, else_blk) ->
      ...
      let u_builder = process_elseifs vars else_blk end_bb curr_func func_blocks var_types else_builder in
      let build_br_end = L.build_br end_bb in
      add_terminal (L.builder_at_end context then_bb) build_br_end;
