...
let rec build_expr expr vars the_module builder =
  ...
  | SList sexprs ->
    let element_type, _ =
      ...
      | [] -> failwith "Cannot create list with no elements"
    in
    let _, sx_list = List.split sexprs in
    let elems = List.map (fun se -> build_expr se vars the_module builder) sx_list in
    (* Build LLVM values for each element *)
    let arr =
      L.build_array_malloc
        element_type
        (L.const_int l_int (List.length elems))
        "arr"
        builder
    in
