...
and fresh_var_name base_name vars counter =
  ...
  if StringSet.mem candidate vars then
    fresh_var_name base_name vars (counter + 1)
  else
    candidate
and unbind_sexpr se replace_self new_var_name =
  ...
  | SUDTAccess (udt_se, udt_member) -> 
    ...
    | SUDTFunction (udt_func_name, udt_func_params) -> 
      let unbound_udt_func_params = List.map (fun param -> unbind_sexpr param replace_self new_var_name) udt_func_params in
      t', SFunctionCall(udt_func_name, unbound_udt_func_params))
...
and unbind_block sblk variables replace_self new_var_name =
  ...
  | SFunctionDefinition (rt, func_name, func_args, body) -> 
    ...
    let updated_variables2, unbound_body = unbind_block_list body updated_variables1 in
  ...
  | SBoundFunctionDefinition (rt, func_name, func_args, body, _) -> 
    let updated_variables1 = add_func_args func_args variables in
    let updated_variables2, _ = unbind_block_list body updated_variables1 in
    let var_name = fresh_var_name "tmp" updated_variables2 0 in
    let updated_func_args = List.map (fun (arg_name, arg_type) -> if arg_name = "self" then (var_name, arg_type) else (arg_name, arg_type)) func_args in
    let updated_variables3 = add_func_args updated_func_args variables in
    let _, unbound_body = unbind_block_list body updated_variables3 in
  ...
  | SIfNonEnd (cond, if_body, other) -> 
    let unbound_cond = unbind_sexpr cond replace_self new_var_name in
    let updated_variables1, unbound_if_body = unbind_block_list if_body variables in
    let updated_variables2, unbound_other = unbind_block other updated_variables1 replace_self new_var_name in
    updated_variables2, SIfNonEnd (unbound_cond, unbound_if_body, unbound_other)
  | SElifNonEnd (cond, elif_body, other) ->
    let unbound_cond = unbind_sexpr cond replace_self new_var_name in
    let updated_variables1, unbound_elif_body = unbind_block_list elif_body variables in
    let updated_variables2, unbound_other = unbind_block other updated_variables1 replace_self new_var_name in
    updated_variables2, SElifNonEnd (unbound_cond, unbound_elif_body, unbound_other)
  | SElifEnd (cond, elif_body) -> 
    let unbound_cond = unbind_sexpr cond replace_self new_var_name in
    let updated_variables, unbound_elif_body = unbind_block_list elif_body variables in
    updated_variables, SElifEnd (unbound_cond, unbound_elif_body)
  | SElseEnd (else_body) -> 
    let updated_variables, unbound_else_body = unbind_block_list else_body variables in
  ...
  | SFor (iterator, iterable_se, for_body) -> 
    ...
    let updated_variables, unbound_for_body = unbind_block_list for_body variables in
...
and unbind_block_list sblk_list variables =
  ...
  | sblk :: rest -> 
    ...
    updated_variables, unbound_sblk :: snd (unbind_block_list rest updated_variables)
;;

let unbind sblk_list = 
  let variables = StringSet.empty in
  let _, unbound_sblk_list = unbind_block_list sblk_list variables in
  unbound_sblk_list
;;